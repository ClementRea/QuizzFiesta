  ...
  try {
    ...
    if (req.user.role === "user") {
      ...
      if (req.user.role === "gestionnaire") {
        ...
        filter.$or = [{ isPublic: true }, { createdBy: { $in: userIds } }];
      }
    }
      ...
      if (req.user.role === "user") {
        ...
        if (filter.$or) {
          filter.$or = filter.$or.map((condition) => {
            if (
              condition.isPublic !== undefined ||
              condition.createdBy !== undefined
            ) {
              return {
                ...condition,
                ...(isPublic === "true"
      ...
      if (req.body.questions) {
        ...
        if (Array.isArray(parsedQuestions)) {
          ...
          if (parsedQuestions.length > 0) {
            const questionPromises = parsedQuestions.map(
              async (questionData) => {
                questionData.quizId = quiz._id;
                const question = new Question(questionData);
                return await question.save({ session });
...
exports.addQuestionsToQuiz = async (req, res, next) => {
  try {
    ...
    try {
      const newQuestionPromises = req.body.questions.map(
        async (questionData) => {
          questionData.quizId = quiz._id;
