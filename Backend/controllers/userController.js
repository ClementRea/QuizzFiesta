...
exports.getUserById = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(req.params.id).select(
      "-refreshTokens -tokenVersion -suspiciousActivity",
    );
    ...
    if (req.user.role === "gestionnaire") {
      if (
        ...
        return res.status(403).json({
          status: "error",
          message:
            "Vous ne pouvez supprimer que les utilisateurs de votre organisation.",
        });
      }
    ...
    if (req.user.role === "gestionnaire") {
      if (
        ...
        return res.status(403).json({
          status: "error",
          message:
            "Vous ne pouvez consulter que les utilisateurs de votre organisation.",
        });
      }
    }
