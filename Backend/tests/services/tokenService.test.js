...
describe('tokenService', () => {
	...
	describe('generateRefreshToken', () => {
		it('génère un refresh token avec uuid de famille', () => {
			...
			const decoded = jwt.verify(token, process.env.JWT_SECRET, { issuer: 'quizzfiesta', audience: 'quizzfiesta-client' });
	...
	describe('hashRefreshToken', () => {
			...
			const ko = await bcrypt.compare(token + 'x', hash);
					if (ko) {
						console.warn('Collision bcrypt improbable détectée dans test (token altéré valide).');
					} else {
	...
	describe('extractSecurityInfo', () => {
		it('extrait user-agent et ip', () => {
			const req = { get: jest.fn().mockImplementation(h => (h === 'User-Agent' ? 'AgentX' : '')), ip: '127.0.0.1', connection: {} };
	...
	describe('detectSuspiciousActivity', () => {
		...
		it('suspect si >5 tokens récents', () => {
			const user = { refreshTokens: [0,1,2,3,4,5].map(i => makeToken(i*5)) };
			const r = tokenService.detectSuspiciousActivity(user, {});
			expect(r.suspicious).toBe(true);
			expect(r.reason).toMatch(/Too many/);
		});
		it('suspect si >3 IP récentes distinctes', () => {
			const user = { refreshTokens: [ makeToken(5,'1'), makeToken(10,'2'), makeToken(15,'3'), makeToken(20,'4') ] };
			const r = tokenService.detectSuspiciousActivity(user, {});
			expect(r.suspicious).toBe(true);
	...
	describe('cleanExpiredTokens', () => {
		it('retire les tokens expirés', () => {
			...
			const user = { refreshTokens: [ { expiresAt: past }, { expiresAt: future } ] };
	...
	describe('invalidateTokenFamily', () => {
		it('retire la famille et incrémente tokenVersion', () => {
			const user = { tokenVersion: 0, refreshTokens: [ { family: 'A' }, { family: 'B' } ] };
