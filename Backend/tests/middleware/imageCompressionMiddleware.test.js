...
describe('imageCompressionMiddleware', () => {
	...
	function buildReqFile({ name = 'photo.jpg', dir = '/tmp', mimetype = 'image/jpeg' } = {}) {
		const filename = name;
	...
	it('compresse un jpg en webp', async () => {
		...
		const expectedCompressed = path.join(path.dirname(req.file.path), 'image.webp');
		expect(instance.toFile).toHaveBeenCalledWith(expectedCompressed);
		expect(unlinkSpy).toHaveBeenCalledWith(expectedCompressed.replace('.webp', '.jpg'));
