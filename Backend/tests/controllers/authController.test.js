...
describe("authController", () => {
  ...
  describe("register", () => {
    ...
    it("should flag suspicious activity and invalidate tokens", async () => {
      ...
      tokenService.detectSuspiciousActivity.mockReturnValue({
        suspicious: true,
        reason: "test",
      });
      req.body = {
      ...
      await register(req, res, next);
      expect(
        require("../../services/tokenService").invalidateAllTokens,
      ).toHaveBeenCalled();
    });
  ...
  describe("refreshToken", () => {
    ...
    it("should return 403 if user not found", async () => {
      ...
      tokenService.verifyRefreshToken.mockReturnValue({
        valid: true,
        payload: { userId: "nope", family: "fam1" },
      });
    ...
    it("should return 403 if family revoked", async () => {
      ...
      tokenService.verifyRefreshToken.mockReturnValue({
        valid: true,
        payload: { userId: "u10", family: "famX" },
      });
      req.body = { refreshToken: "good" };
      const userMock = {
        _id: "u10",
        refreshTokens: [
          { family: "famY", expiresAt: new Date(Date.now() + 10000) },
        ],
        save: jest.fn(),
      };
    ...
    it("should refresh successfully", async () => {
      ...
      tokenService.verifyRefreshToken.mockReturnValue({
        valid: true,
        payload: { userId: "u11", family: "famZ" },
      });
      req.body = { refreshToken: "good" };
      const tokenRecord = {
        family: "famZ",
  ...
  describe("logout", () => {
    ...
    it("should logout all sessions", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith(
        expect.objectContaining({ message: "Logged out from all sessions" }),
      );
    });
    ...
    it("should logout single session family", async () => {
      ...
      tokenService.verifyRefreshToken.mockReturnValue({
        valid: true,
        payload: { userId: "u2", family: "fam2" },
      });
      const userMock = { _id: "u2", refreshTokens: [], save: jest.fn() };
      User.findById.mockResolvedValue(userMock);
      req.body = { refreshToken: "tok", logoutAll: false };
      await logout(req, res, next);
      expect(tokenService.invalidateTokenFamily).toHaveBeenCalledWith(
        userMock,
        "fam2",
      );
      expect(res.json).toHaveBeenCalledWith(
