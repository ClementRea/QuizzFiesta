...
describe('gameSessionController', () => {
  let req, res;
  beforeEach(() => {
    req = { params: {}, body: {}, user: { id: 'user123', userName: 'User', avatar: 'avatar.png' } };
  ...
  describe('createGameSession', () => {
    it('creates session success', async () => {
      ...
      const session = { _id: 's1', sessionCode: 'ABC123', status: 'lobby', quiz: {}, organizer: {}, settings: {}, participantCount: 1, gameState: {}, createdAt: new Date(), populate: jest.fn().mockResolvedValue(), save: jest.fn().mockResolvedValue() };
    ...
    it('500 on internal error', async () => {
      ...
      GameSession.createSession = jest.fn().mockRejectedValue(new Error('fail'));
      await gameSessionController.createGameSession(req, res);
  ...
  describe('joinSessionByCode', () => {
    const baseSession = () => ({ _id: 's1', sessionCode: 'CODE', status: 'lobby', quiz: {}, organizer: {}, settings: {}, participantCount: 0, gameState: {}, canJoin: jest.fn().mockReturnValue(true), canLateJoin: jest.fn().mockReturnValue(false) });
    it('success join', async () => {
      req.params.sessionCode = 'CODE';
      GameSession.findOne.mockReturnValue({ populate: jest.fn().mockResolvedValue(baseSession()) });
    ...
    it('400 cannot join (finished)', async () => {
      ...
      GameSession.findOne.mockReturnValue({ populate: jest.fn().mockResolvedValue(sess) });
      LobbyParticipant.findOne.mockResolvedValue(null);
      await gameSessionController.joinSessionByCode(req, res);
  ...
  describe('joinSessionLobby', () => {
    it('joins new participant', async () => {
      ...
      const session = { _id: 's1', organizerId: 'user123', canJoin: jest.fn().mockReturnValue(true), updateParticipantCount: jest.fn(), quizId: 'q1', participantCount: 1, settings: {}, status: 'lobby' };
      GameSession.findById.mockResolvedValue(session);
      LobbyParticipant.findOne.mockResolvedValue(null);
      const save = jest.fn();
      LobbyParticipant.prototype.save = save;
  LobbyParticipant.find.mockReturnValue({ select: jest.fn().mockResolvedValue([{ userId: 'user123', userName: 'User', avatar: 'a', isReady: true, isOrganizer: true, connectionStatus: 'connected' }]) });
      await gameSessionController.joinSessionLobby(req, res);
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ status: 'success' }));
    });
    it('reconnect existing participant', async () => {
      req.params.sessionId = 's1';
      const participant = { connectionStatus: 'disconnected', lastSeen: null, save: jest.fn() };
      GameSession.findById.mockResolvedValue({ _id: 's1', organizerId: 'user123', canJoin: jest.fn().mockReturnValue(true), updateParticipantCount: jest.fn(), quizId: 'q1', participantCount: 1, settings: {}, status: 'lobby' });
      LobbyParticipant.findOne.mockResolvedValue(participant);
  LobbyParticipant.find.mockReturnValue({ select: jest.fn().mockResolvedValue([]) });
      await gameSessionController.joinSessionLobby(req, res);
    ...
    it('400 cannot join', async () => {
      req.params.sessionId = 's1';
      GameSession.findById.mockResolvedValue({ _id: 's1', canJoin: jest.fn().mockReturnValue(false) });
      await gameSessionController.joinSessionLobby(req, res);
      expect(res.status).toHaveBeenCalledWith(400);
    });
    it('404 not found', async () => {
  ...
  describe('leaveSessionLobby', () => {
    it('leave and cancel when organizer alone', async () => {
      req.params.sessionId = 's1';
      const session = { _id: 's1', organizerId: 'user123', updateParticipantCount: jest.fn(), save: jest.fn(), status: 'lobby' };
      GameSession.findById.mockResolvedValue(session);
  ...
  describe('getSessionParticipants', () => {
    ...
    it('500 error', async () => {
      req.params.sessionId = 's1';
      LobbyParticipant.find.mockImplementation(() => { throw new Error('x'); });
      await gameSessionController.getSessionParticipants(req, res);
      expect(res.status).toHaveBeenCalledWith(500);
    });
  });
  ...
  describe('setParticipantReady', () => {
    it('success', async () => {
      req.params.sessionId = 's1'; req.body.isReady = true;
  ...
  describe('startGameSession', () => {
    ...
    it('403 not organizer', async () => {
      req.params.sessionId = 's1';
      GameSession.findById.mockResolvedValue({ _id: 's1', organizerId: 'other', status: 'lobby' });
      await gameSessionController.startGameSession(req, res);
    ...
    it('success start', async () => {
      req.params.sessionId = 's1';
      const session = { _id: 's1', organizerId: 'user123', status: 'lobby', startGame: jest.fn(), gameState: {}, sessionCode: 'C', startedAt: new Date() };
      GameSession.findById.mockResolvedValue(session);
      LobbyParticipant.countDocuments.mockResolvedValue(1);
      LobbyParticipant.find.mockResolvedValue([{ userId: 'user123', userName: 'User', avatar: 'a' }]);
  ...
  describe('getSessionState', () => {
    it('success', async () => {
      req.params.sessionId = 's1';
      GameSession.findById.mockReturnValue({ populate: jest.fn().mockResolvedValue({ _id: 's1' }) });
      await gameSessionController.getSessionState(req, res);
      expect(res.json).toHaveBeenCalled();
    });
    it('404 missing', async () => {
      req.params.sessionId = 's1';
      GameSession.findById.mockReturnValue({ populate: jest.fn().mockResolvedValue(null) });
      await gameSessionController.getSessionState(req, res);
      expect(res.status).toHaveBeenCalledWith(404);
    });
  });
  ...
  describe('endGameSession', () => {
    ...
    it('403 not organizer', async () => {
      req.params.sessionId = 's1';
      GameSession.findById.mockResolvedValue({ _id: 's1', organizerId: 'other' });
      await gameSessionController.endGameSession(req, res);
      expect(res.status).toHaveBeenCalledWith(403);
    });
    it('success end', async () => {
      req.params.sessionId = 's1';
      const session = { _id: 's1', organizerId: 'user123', endSession: jest.fn() };
      GameSession.findById.mockResolvedValue(session);
