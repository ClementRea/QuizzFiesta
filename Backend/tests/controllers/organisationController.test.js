...
jest.mock("../../middlewares/imageCompressionMiddleware", () => ({
  compressImage: (req, res, next) => next(),
}));
  ...
  describe("addMembersToOrganisation", () => {
    it("ajoute membres", async () => {
      ...
      const req = {
        ...
        body: { memberIds: [newUser._id.toString()] },
        user: { _id: owner._id },
      };
    ...
    it("refus non propriétaire", async () => {
      ...
      const req = {
        params: { id: org._id.toString() },
        body: { memberIds: [victim._id.toString()] },
        user: { _id: other._id },
      };
      await organisationController.addMembersToOrganisation(req, res, next);
  ...
  describe("removeMembersFromOrganisation", () => {
    it("retire membre", async () => {
      ...
      const org = await Organisation.create({
        ...
        createdBy: owner._id,
        ownerId: owner._id,
        members: [owner._id, m._id],
      });
      const req = {
        params: { id: org._id.toString() },
        body: { memberIds: [m._id.toString()] },
        user: { _id: owner._id },
      };
      await organisationController.removeMembersFromOrganisation(
        req,
        res,
        next,
      );
    ...
    it("interdit retrait propriétaire", async () => {
      ...
      const req = {
        params: { id: org._id.toString() },
        body: { memberIds: [owner._id.toString()] },
        user: { _id: owner._id },
      };
      await organisationController.removeMembersFromOrganisation(
        req,
