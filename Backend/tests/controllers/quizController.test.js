...
async function createQuiz({ creator, questions = [], isPublic = true }) {
  ...
  for (const q of questions) {
    const nq = await Question.create({
      quizId: quiz._id,
      content: q.content,
      type: q.type || "CLASSIC",
      answer: q.answer || [{ text: "A", isCorrect: true }],
      points: q.points || 10,
      timeGiven: q.timeGiven || 30,
    });
    qIds.push(nq._id);
  }
  if (qIds.length) {
...
describe("quizController (integration)", () => {
  ...
  describe("getQuizById", () => {
    ...
    it("200 accès créateur", async () => {
      ...
      const req = {
        params: { id: quiz._id.toString() },
        user: { id: creator._id.toString(), role: "user" },
      };
      await quizController.getQuizById(req, res, next);
      expect(res.status).toHaveBeenCalledWith(200);
    });
    it("200 accès via lobby", async () => {
      ...
      const req = {
        params: { id: quiz._id.toString() },
        user: { id: watcher._id.toString(), role: "user" },
      };
      await quizController.getQuizById(req, res, next);
      expect(res.status).toHaveBeenCalledWith(200);
    });
  ...
  describe("quizUpdate", () => {
    ...
    it("200 update simple", async () => {
      ...
      const req = {
        params: { id: quiz._id.toString() },
        user: { id: creator._id.toString(), role: "user" },
        body: { title: "Modif", description: "D2" },
      };
      await quizController.quizUpdate(req, res, next);
      expect(res.status).toHaveBeenCalledWith(200);
      const payload = res.json.mock.calls[0][0];
  ...
  describe("addQuestionsToQuiz", () => {
    it("404 quiz inexistant", async () => {
      ...
      const req = {
        ...
        body: {
          questions: [
            { content: "X", answer: [{ text: "A", isCorrect: true }] },
          ],
        },
      };
    ...
    it("403 ajout non autorisé", async () => {
      ...
      const req = {
        ...
        body: {
          questions: [
            { content: "N", answer: [{ text: "A", isCorrect: true }] },
          ],
        },
      };
      await quizController.addQuestionsToQuiz(req, res, next);
